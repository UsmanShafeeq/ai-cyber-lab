...
st.title("🛡️ AI Cybersecurity Dashboard (SOC Simulation)")
...
clf = joblib.load("models/supervised_model.pkl")
...
anomaly_protocol_encoder = joblib.load("models/anomaly_protocol_encoder.pkl")
...
num_events = st.sidebar.slider("Number of Events", 20, 200, 50, 10)
interval = st.sidebar.slider("Interval (sec)", 0.1, 2.0, 1.0, 0.1)
attack_ratio = st.sidebar.slider("Attack Ratio", 0.01, 0.5, 0.08, 0.01)
...
def simulate_event():
    ...
    packet_size = np.random.normal(700, 300)
    packet_size = max(40, min(9000, int(packet_size)))
    connection_duration = np.random.exponential(2)
    source_port = (
